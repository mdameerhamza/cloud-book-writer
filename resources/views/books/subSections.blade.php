        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    @can('create-sections')
                        <button class="text-indigo-600 hover:text-indigo-900 ml-4" id="addSectionBtn">Add Sub Section</button>
                    @endcan

                    @foreach ($subsections as $subsection)
                        <div class="mb-4">
                            <h1 class="text-3xl font-semibold">
                                <a href="{{ route('books.subsections', ['book' => $subsection->book_id, 'section' => $subsection->id]) }}" class="text-indigo-600 hover:text-indigo-900">{{ $subsection->title }}</a>
                            </h1>
                            <p class="text-lg">{{ $subsection->description }}</p>
                        </div>

                        @can('edit-sections')
                            <button class="text-indigo-600 hover:text-indigo-900 ml-2 editSectionBtn" data-section-id="{{ $subsection->id }}">Edit</button>
                        @endcan
                    @endforeach
                </div>
            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addSectionModalLabel">Add Sub Section</h5>
...
<script>
    ...
    $('#saveSectionBtn').click(function () {
        ...
        $.ajax({
            url: "{{ route('sub.sections.store') }}", // Replace with your route for creating sections
            method: 'POST',
            data: formData,
            headers: {
